...
public class ProtoFileDescriptorSetLoader implements Loader {
    ...
    public List<Descriptors.Descriptor> loadDescriptors() throws Descriptors.DescriptorValidationException {
        var descriptors = loadFileDescriptors();
    ...
    @Override
    public Map<String, Descriptors.FileDescriptor> loadFileDescriptors() throws DescriptorValidationException {
        // we need to build a DAG of filenames so that we can build the Descriptors.Descriptor objects
    ...
    private List<Descriptors.Descriptor> collectAllMessageDescriptors
        ...
        for (var fileDescriptor : fileDescriptors) {
            descriptors.addAll(collectAllNestedMessageDescriptors(fileDescriptor.getMessageTypes().stream().toList()));
        }
        return descriptors;
    }

    public List<Descriptors.Descriptor> collectAllNestedMessageDescriptors
        ...
        for (var descriptor : descriptors) {
            if (descriptor.getNestedTypes().isEmpty()) {
                ...
                descriptorList.addAll(collectAllNestedMessageDescriptors(descriptor.getNestedTypes()));
                descriptorList.add(descriptor);
            }
        }
